...
class Detector(torch.nn.Module):
    ...
    def __init__(self, layers=[16, 32, 32], n_class=4, kernel_size=3, use_skip=True):
        """
           Your code here.
        ...
        self.depth = torch.nn.Conv2d(c, 1, 1)
        self.size = torch.nn.Conv2d(c, 2, 1)
        self.is_puck = torch.nn.Conv2d(c, 1, 1)
    ...
    def forward(self, x):
        ...
        return self.classifier(z), self.size(z), self.depth(z), F.adaptive_max_pool2d(self.is_puck(z), output_size=1)[:,0,0,0]
    ...
    def detect(self, image, **kwargs):
        """
           ...
           Hint: Make sure to return three python lists of tuples of (float, int, int, float, float) and not a pytorch
                 ...
                 out of memory.
        """
        params = [{'max_det':2, 'min_score':-1}, {'max_det': 1, 'min_score':-1000}, {'max_det': 3, 'min_score': 0}, {'max_det': 3, 'min_score': 1}]
        cls, size, depth, is_puck = self.forward(image[None])
        size = size.cpu()
        return [[(s, x, y, float(size[0, 0, y, x]), float(size[0, 1, y, x]))
                 for s, x, y in extract_peak(c, max_pool_ks=15, **{**kwargs, **params[i]})] for i, c in enumerate(cls[0])], depth, is_puck.item()
...
if __name__ == '__main__':
    ...
    dataset = DetectionSuperTuxDataset('tournament_puck2', min_size=0)
    ...
    for i, ax in enumerate(axs.flat):
        im, depth, kart, bomb, pickup, _ = dataset[90 + i]
